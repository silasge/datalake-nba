...
def run_nba_bronze():
    ...
    if args.shotchartdetail:
        ...
        for sy, tid, pid in (pbar := tqdm(season_team_player_id)):
            pbar.set_description(f"Processing {sy}-{tid}-{pid}")
            insert_shot_chart_detail(season_year=sy, team_id=tid, player_id=pid)
